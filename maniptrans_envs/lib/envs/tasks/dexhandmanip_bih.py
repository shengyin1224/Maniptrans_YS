...
class DexHandManipBiHEnv(VecTask):
    ...
    def _create_envs(self):
        ...
        # 1. 仅加载不重复的 Unique 数据 (通常只有几个或几十个)
        unique_data_lh_list = []
        unique_data_rh_list = []
        ...
        for data_idx in self.dataIndices:
            ...
            # 获取 LH 数据
            unique_data_lh_list.append(self.demo_dataset_lh_dict[dataset_type][data_idx])
            # 获取 RH 数据
            unique_data_rh_list.append(self.demo_dataset_rh_dict[dataset_type][data_idx])
        ...
        # 4. 广播数据 (Broadcast)
        print(f"Broadcasting data to {self.num_envs} environments...")
    ...
    def pack_data(self, data, side="rh"):
        ...
        def fill_data(stack_data):
            ...
            # === [修改] 安全的堆叠逻辑 ===
            res = torch.stack(stack_data) # 形状是 [Batch, Time, Dim]
            return res
    ...
    def _create_obj_actor(self, env_ptr, i, current_asset, side="rh"):
        ...
        if side == "rh":
            if "scene_objs" in self.demo_data_rh:
                scene_objs = self.demo_data_rh["scene_objs"][i]
            else:
                scene_objs = [] # Humoto 没有杂物，给个空列表
        else:
            if "scene_objs" in self.demo_data_lh:
                scene_objs = self.demo_data_lh["scene_objs"][i]
            else:
                scene_objs = [] # Humoto 没有杂物，给个空列表
